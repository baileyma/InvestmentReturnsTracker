...
public class AggregateAccountsServiceTests
{
    ...
    {
        _accountsRepo.GetAccountsByUserId(Arg.Any<string>()).Returns(_fixture.Create<List<Account>>());
        _balancesRepo.GetBalancesByUserIdAsync(Arg.Any<string>()).Returns(_fixture.Create<List<Balance>>());
        _paymentsRepo.GetAllPaymentsAsync(Arg.Any<string>()).Returns(_fixture.Create<List<Payment>>());
