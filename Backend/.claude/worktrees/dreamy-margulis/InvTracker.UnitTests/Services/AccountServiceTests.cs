...
public class AccountServiceTests
{
  ...
  {
        _balancesRepository.GetBalanceByIdAndYear(Arg.Any<int>(), Arg.Any<int>()).Returns(_fixture.Create<Balance>());
        _paymentsRepository.GetPaymentsByIdAndYear(Arg.Any<int>(), Arg.Any<int>()).Returns(_fixture.Create<List<Payment>>());
        ...
        var result = await sut.GetAccountDetailsByYear(1, 2024);
