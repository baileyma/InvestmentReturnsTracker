...
public class ReturnsController : ControllerBase
{
    ...
    public async Task<ActionResult<AccountYearlyDetail>> GetAccountYearlyDetails(int accountId, int year)
    {
        try
        {
            if (_cache.TryGetValue($"accountyearlyreturns_{accountId}_{year}", out AccountYearlyDetail? cachedReturns))
            {
                _logger.CacheUsed("AccountPage");
                return cachedReturns;
            }

            var accountYearlyDetail = _accountService.GetAccountDetailsByYear(accountId, year);
            
            var cacheEntryOptions = new MemoryCacheEntryOptions().SetSlidingExpiration(TimeSpan.FromSeconds(200));

            await _cache.Set($"accountyearlyreturns_{accountId}_{year}", accountYearlyDetail, cacheEntryOptions);
    ...
    {
        if (_cache.TryGetValue(("homePageData"), out HomePageData? cachedResult))
        {
        ...
        {
            AggregateData = await _aggregateAccountService.GetAggregateData(),
            IndividualAccountData = await _aggregateAccountService.GetIndividualAccountsData()
        };
