...
{
    Task<Dictionary<int, AggregateData>> GetAggregateData();

    Task<Dictionary<int, AccountReturn>> GetIndividualAccountsData();
}
...
public class AggregateAccountsService : IAggregateAccountService
{
    ...
    public async Task<Dictionary<int, AggregateData>> GetAggregateData()
    {
        // DRY
    ...
    public async Task<Dictionary<int, AccountReturn>> GetIndividualAccountsData()
    {
        var years = Enumerable.Range(2021, DateTime.Today.Year - 2020);
        var accounts = await _accountsRepo.GetAccounts();
        var balances = await _balancesRepo.GetAllBalances();
